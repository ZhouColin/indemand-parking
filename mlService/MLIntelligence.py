...
def clustering(data):
	...
	demandCoords = zip([c.lat for c in supply], [c.lon for c in supply])
	demandCoords = pd.as_matrix(demandCoords)
	...
	db = dbObj.fit(np.radians(demandCoords))
	cluster_labels = db.labels_
	num_clusters = len(set(cluster_labels))
	clusters = pd.Series([demandCoords[cluster_labels == n] for n in range(num_clusters)])  #unsure about this line...seems like youre just taking the first n entries in demandCoords?
